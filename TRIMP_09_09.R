...
totime <- function(sec){
  th <- sec %/% 3600
  tm <- (sec - (3600*th)) %/% 60 %>% sprintf("%02d", .)
  ts <- sec %% 60 %>% sprintf("%02d", .)
  ar <- array(c(th,tm,ts))
  return(ar)
}
#totime(4523)
...
trimp_z_fct <- function(obj) {
  ...
  act_hr<- lapply(num, function(x){
    dieter_train@activity[[x]]@heart_rate
  })
...
dieter<- athlete(90, 193, sex="male", name="Dieter")
...
act_p<-activity(data[[1]])
paul_1 <- training(list(act_p), paul)
...
setGeneric(name="plot_trimp_zone",
           def=function(obj, num=1)
           {
             standardGeneric("plot_trimp_zone")
           }
)

setMethod(f="plot_trimp_zone",
          signature="training",
          definition=function(obj, num=1)
          {
...
barplot(table(carb), horiz=TRUE, col="lightgreen", xlab="Frequency", ylab="Range") +
  title(main="Horizontal Bar Plot", font.main= 4)


as.Date(dieter_train@activity[[1]]@time)[1]

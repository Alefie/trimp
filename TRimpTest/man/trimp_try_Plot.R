...
run_2 <- readTCX(file = "tcx/b_2.tcx", timezone = "UTC")
...
for(i in 1:8) {
    ...
    addMarkers(m, lng=tr@activity[[i]]@longitude[1], lat=tr@activity[[i]]@latitude[1], popup="run_1_start") %>%
    addMarkers(m, lng=tr@activity[[i]]@longitude[length(tr@activity[[i]]@longitude)], lat=tr@activity[[i]]@latitude[length(tr@activity[[i]]@latitude)], popup="run_1_end") %>%
    addPolygons(m, lng=tr@activity[[i]]@longitude, lat=tr@activity[[i]]@latitude, popup="Run_1")
     # Print the map
}
print(m)
...
#Ergebnis:
setGeneric(name="plotroute",
           def=function(train)
           {
             standardGeneric("plotroute")
           }
)
...
setMethod(f="plotroute",
          ...
          definition=function(train)
          {
            m <- leaflet() %>%
                ...
                #supports: matrices, data frames, spatial objects
                addMarkers(lng=train@activity[[1]]@longitude[1], lat=train@activity[[1]]@latitude[1], popup="run_1_start")  %>%
                addMarkers(lng=train@activity[[1]]@longitude[length(train@activity[[1]]@longitude)], lat=train@activity[[1]]@latitude[length(tr@activity[[1]]@latitude)], popup="run_1_end")  %>%
                addPolygons(lng=train@activity[[1]]@longitude, lat=train@activity[[1]]@latitude, popup="Run_1")
            print(m)
          }
)
...
#klappt, aber leider mit for loop
setMethod(f="plotroute",
          signature="training",
          definition=function(train)
          {
            m <- leaflet()
            for (i in 1:2) { #nur die ersten beiden Läufe erstmal
              m <- addTiles(m) %>%  # Add default OpenStreetMap map tiles
              #supports: matrices, data frames, spatial objects
              addMarkers(m, lng=train@activity[[i]]@longitude[1], lat=train@activity[[i]]@latitude[1], popup="run_1_start")  %>%
              addMarkers(m, lng=train@activity[[i]]@longitude[length(train@activity[[i]]@longitude)], lat=train@activity[[i]]@latitude[length(tr@activity[[i]]@latitude)], popup="run_1_end")  %>%
              addPolygons(m, lng=train@activity[[i]]@longitude, lat=train@activity[[i]]@latitude, popup="Run_1")
            }
            print(m)
          }
)

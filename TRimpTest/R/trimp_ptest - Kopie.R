...
setwd("C:/Users/Lisa/Desktop/09_09_1615/trimp-master")
...
setMethod(f="plot_starts",
          ...
          {
            num <- 1:length(obj)
            long <- lapply(num, function(x){obj[[x]]@longitude[1]})
            lati <- lapply(num, function(x){obj[[x]]@latitude[1]})
            ti <- lapply(num, function(x){totime(obj[[x]]@total_time)})
            m <- leaflet() %>%
              addTiles() %>%  # Add default OpenStreetMap map tiles
              #supports: matrices, data frames, spatial objects
